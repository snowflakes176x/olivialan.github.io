...
.ready(
		function() {
			...
			CC.prototype.display = function() {
				...
				/*
				 * if(this.m == "C"){ this.c2.type = "arc"; }else if(this.c2.m ==
				 * "B"){ this.c2.type = "line"; }
				 */
				if (this.c2.type == "arc") {
					main.beginPath();
			...
			(function draw() {
				...
				for ( var i = 0; i < PPArray.length; i++) {
					if (clickObject == null
						...
						PPArray[i].p2.weaken = false;
					    /*
						 * if (clickObject != null && PPArray[i].p1.id ==
						 * clickObject.id) { PPArray[i].p1.connectParent() } if
						 * (clickObject != null && PPArray[i].p2.id ==
						 * clickObject.id) { PPArray[i].p2.connectParent() }
						 */
					}
				}
				for ( var i = 0; i < CPArray.length; i++) {
			...
			function CCMLegend(x, y, space) {
				for ( var i = 0; i < CCMColor.length; i++) {
					if(CCMColor[i].name !=""){
						...
						/*
						 * main.fillStyle = "#fff"; main.beginPath();
						 * main.arc(x, y, 2.5, 0, Math.PI * 2); main.fill();
						 */
						main.fillStyle = "#ccc";
						main.font = "12px 'Microsoft YaHei', 微软雅黑";
